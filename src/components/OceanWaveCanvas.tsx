...
const LAYERS: WaveLayer[] = [
  { freq: 1.2,  amp: 0.09,  speed: 0.35, color: '#06b6d4', opacity: 0.60 },
  { freq: 1.6,  amp: 0.065, speed: 0.25, color: '#0891b2', opacity: 0.45 },
  { freq: 2.0,  amp: 0.05,  speed: 0.45, color: '#22d3ee', opacity: 0.32 },
  { freq: 2.6,  amp: 0.04,  speed: 0.55, color: '#67e8f9', opacity: 0.22 },
];
...
export default function OceanWaveCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      // waves start from ~35% height so crests can overflow upward
      const baseY = h * 0.38;
      ...
      // foam particles — can go above the container visually
      const dtScale = dt * 60;
      ...
      for (let i = foams.length - 1; i >= 0; i--) {
        ...
        ctx.beginPath();
        ctx.arc(f.x * w, f.y, f.r * (1 - progress * 0.3), 0, Math.PI * 2);

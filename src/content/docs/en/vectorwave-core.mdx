...
See the [API Reference](/docs/vectorwave-api) for all `@trace_span` parameters.
...
- **`semantic_cache=True`** automatically enables `capture_return_value=True` — cached results need stored return values.
- **`replay=True`** automatically enables `capture_return_value=True` and captures all function arguments — replay testing needs full input/output data.

...
3. If self-healing is configured, triggers the `AutoHealerBot`
...
Tag functions by team for filtering and access control:

```python
@vectorize(team="ml-team", auto=True)
def ml_function():
    ...

@vectorize(team="backend-team", auto=True)

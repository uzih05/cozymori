...
The [Drift Radar](/docs/vectorwave-drift) calculates a **centroid** (average vector) from all Golden entries for each function. New inputs are compared against this centroid — when the distance exceeds the threshold, a drift alert is triggered.

More Golden data = more accurate centroid = fewer false positives.

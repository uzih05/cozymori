...
[드리프트 레이더](/docs/vectorwave-drift)는 각 함수의 모든 Golden 항목에서 **중심점**(평균 벡터)을 계산합니다. 새 입력은 이 중심점과 비교되어, 거리가 임계값을 초과하면 드리프트 알림이 발생합니다.

Golden 데이터가 많을수록 = 더 정확한 중심점 = 오탐 감소.

...
# 함수의 최근 20건의 실행을 리플레이 (Golden Data 우선)
...
리플레이어는 자동으로 Golden Dataset 항목을 우선 사용하고, 그 다음 일반 실행 로그를 사용합니다.

### 시맨틱 비교

AI/생성형 함수에서 출력이 달라질 수 있는 경우, `SemanticReplayer`를 사용하세요:
...
results = semantic_replayer.replay(
    ...
    semantic_eval=True,          # LLM 기반 시맨틱 평가
)
```

적합한 경우: 생성형 AI, NLP 출력, 요약.

결정적 함수 (수학 연산, 데이터 변환)에는 정확 일치 비교를 수행하는 기본 `VectorWaveReplayer`를 사용하세요.
...
results = semantic_replayer.replay(
    function_full_name="app.generate_response",
    limit=5,
    similarity_threshold=0.70,  # 낮은 임계값, 합리적인 결과인지만 확인
)
...
results = semantic_replayer.replay(
    function_full_name="app.generate_response",
    limit=100,
    similarity_threshold=0.90,  # 엄격한 임계값
    semantic_eval=True,
)

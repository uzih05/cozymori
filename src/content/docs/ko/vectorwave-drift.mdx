...
## 기준선으로서의 Golden Dataset

드리프트 감지는 [Golden Dataset](/docs/vectorwave-golden)을 기준선으로 사용합니다. 함수의 모든 Golden 항목의 중심점(평균 벡터)이 "정상" 영역을 정의합니다. 새로운 입력은 이 중심점과 비교됩니다.

드리프트 감지를 위한 Golden 데이터 설정은 [Golden Dataset](/docs/vectorwave-golden)을 참고하세요.
...
> **VectorSurfer**: 드리프트 모니터링은 [VectorSurfer 대시보드](/vectorsurf)에서 시각화됩니다 — 함수별 실시간 드리프트 거리 차트, 알림 이력, 중심점 시각화.

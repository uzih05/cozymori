...
입력이 정상 패턴에서 벗어나는 것을 모니터링합니다. 새 입력이 Golden Dataset 중심점에서 드리프트 임계값을 초과하면, VectorWave가 웹훅 알림(Discord, Slack 등)을 전송합니다.
...
| `VECTORWAVE_VECTORIZER` | 아니오 | `openai_client` 또는 `huggingface` (기본값: `huggingface`) |
| `GITHUB_TOKEN` | 자동 PR용 | 자동 치유 PR 생성을 위한 GitHub 토큰 |
| `GITHUB_REPO` | 자동 PR용 | 대상 리포지토리 (예: `org/repo-name`) |

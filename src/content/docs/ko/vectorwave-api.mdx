...
| `semantic_cache_scope` | `str` | `None` | 캐시 격리 스코프 (프로젝트/사용자 수준 멀티테넌시) |
...
| `team` | `str` | `None` | 필터링 및 접근 제어를 위한 팀 태그 |
| `tags` | `dict` | `None` | 실행 태깅을 위한 키-값 메타데이터 태그 |
| `drift_threshold` | `float` | `0.25` | 시맨틱 드리프트 감지 임계값 |
...
Weaviate 컬렉션을 생성합니다. 최초 사용 전 한 번 호출해야 합니다.

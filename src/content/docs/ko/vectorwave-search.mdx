...
자연어 설명과의 시맨틱 유사도로 에러를 찾을 수 있습니다:
...
`search_executions`(필터 기반)와 달리, `search_errors_by_message`는 **벡터 검색**을 사용하여 의미적으로 유사한 에러 메시지를 찾습니다 — 정확한 에러 텍스트를 모를 때 유용합니다.
...
results = search_functions_hybrid(
    ...
    limit=10,
    alpha=0.5,   # 0.0 = 순수 키워드 (BM25), 1.0 = 순수 벡터
    filters={"team": "backend"},  # 선택적 필터
)
```

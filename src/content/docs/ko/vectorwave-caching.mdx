...
기본적으로 캐시는 전역입니다. `semantic_cache_scope`를 사용하여 런타임에 함수 인자에서 필터 값을 동적으로 추출하여 캐시를 격리할 수 있습니다:
...
@vectorize(
    semantic_cache=True,
    cache_threshold=0.95,
    semantic_cache_scope=["user_id"],  # 함수 인자에서 추출
)
...
@vectorize(
    semantic_cache=True,
    semantic_cache_scope=["user_id", "region"],  # 여러 스코프 키
)
def regional_query(user_id: str, region: str, query: str):
    return llm.complete(query, region=region)
```

`semantic_cache_scope=["user_id"]`이고 함수가 `fn(user_id="U123", query="...")`으로 호출되면, 캐시 조회가 자동으로 `{"user_id": "U123"}`으로 필터링됩니다. 이를 통해 각 사용자가 독립된 캐시를 사용합니다.

함수 인자가 아닌 정적 필터에는 `semantic_cache_filters`를 사용하세요:
...
- [OpenAI API 문서](https://platform.openai.com/docs) — 시맨틱 유사도에 사용되는 임베딩 API
